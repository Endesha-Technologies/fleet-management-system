...
import type { PostReplacementDialogProps } from '../../_types';

export function PostReplacementDialog({ open, onOpenChange, removedTyres, truckOdometer, onComplete }: PostReplacementDialogProps) {
  if (!open) return null;
  ...
  return (
    <>
      ...
      />
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl z-[70] max-w-4xl w-full mx-4 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  ...
                  <th className="px-4 py-3 text-left font-medium text-gray-700">Km on Truck</th>
              ...
              <tbody>
                {removedTyres.map((tyre, idx) => (
                  <tr key={idx} className="border-b border-gray-200 last:border-0 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-700">{parseInt(truckOdometer).toLocaleString()} km</td>
                    <td className="px-4 py-3 text-gray-700">{(tyre.kmOnTyre || 0).toLocaleString()} km</td>
