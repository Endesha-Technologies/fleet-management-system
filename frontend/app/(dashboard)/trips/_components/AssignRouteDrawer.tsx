...
import { TripRoute, RouteWaypoint } from '@/types/trip';
...
import type { AssignRouteDrawerProps, AssignRouteData } from '../_types';
...
export function AssignRouteDrawer({ open, onClose, trip, onAssign }: AssignRouteDrawerProps) {
  ...
  const routeOptions = [
    ...
    ...MOCK_ROUTES.map((route) => ({
      value: route.id,
      label: `${route.name} (${route.origin.name} → ${route.destination.name})`,
    })),
  ];
